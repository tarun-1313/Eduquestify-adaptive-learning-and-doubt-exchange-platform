import jwt from 'jsonwebtoken';

// The JWT secret is hardcoded in the auth.ts file
const JWT_SECRET = Buffer.from('', 'hex');

async function createTestDownloads() {
  try {
    // Create a test JWT token using an existing user
    const testUser = { id: 1, email: 'kakaji131919@gmail.com', name: 'User 1' };
    const token = jwt.sign(testUser, JWT_SECRET, { algorithm: 'HS256', expiresIn: '1h' });
    
    const testDownloads = [
      { questionBankId: 2, subject: 'Physics', topic: 'Mechanics', department: 'Engineering', year: '2024' },
      { questionBankId: 3, subject: 'Chemistry', topic: 'Organic Chemistry', department: 'Science', year: '2023' },
      { questionBankId: 4, subject: 'Mathematics', topic: 'Algebra', department: 'Engineering', year: '2024' },
      { questionBankId: 5, subject: 'Computer Science', topic: 'Data Structures', department: 'Engineering', year: '2024' },
      { questionBankId: 6, subject: 'Physics', topic: 'Thermodynamics', department: 'Engineering', year: '2023' }
    ];
    
    console.log('Creating multiple test downloads...');
    
    for (const downloadData of testDownloads) {
      const response = await fetch('http://localhost:3002/api/question-banks/download', {
        method: 'POST',
        headers: {
          'Cookie': `eduq_token=${token}`,
          'Content-Type': 'application/json'
        },
        body: JSON.stringify(downloadData)
      });
      
      const data = await response.json();
      console.log(`Download ${downloadData.subject} - ${downloadData.topic}:`, response.status, data.message);
      
      // Small delay between requests
      await new Promise(resolve => setTimeout(resolve, 100));
    }
    
    console.log('✅ All test downloads created successfully');
    
  } catch (error) {
    console.error('Test failed:', error.message);
  }
}

createTestDownloads();